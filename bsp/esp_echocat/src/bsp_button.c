...
#include "bsp/esp_echocat.h"

static const char *TAG = "ESP-EchoCat";
...
static touch_channel_handle_t bsp_touchpad_chan_handles[TOUCH_MAX_CHAN_ID + 1] = { NULL };
static uint32_t bsp_touchpad_last_val[TOUCH_MAX_CHAN_ID];
...
static esp_err_t bsp_touchpad_custom_init()
{
    ...
    for (int i = 0; i < BSP_BUTTON_NUM; i++) {
        if (bsp_button_config[i].type == BSP_BUTTON_TYPE_TOUCHPAD) {
            ...
            ESP_GOTO_ON_ERROR(touch_sensor_new_channel(bsp_touchpad_sens_handle,
                              chan, &chan_cfg, &bsp_touchpad_chan_handles[chan]),
                              err, TAG, "Touch sensor new channel failed");
        }
    }

    /* Default filter */
    touch_sensor_filter_config_t filter_cfg = TOUCH_SENSOR_DEFAULT_FILTER_CONFIG();
    ESP_GOTO_ON_ERROR(touch_sensor_config_filter(bsp_touchpad_sens_handle, &filter_cfg),
                      err, TAG, "Touch sensor config filter failed");

    /* Enable the touch sensor to do the initial scanning, so that to initialize the channel data */
    ESP_GOTO_ON_ERROR(touch_sensor_enable(bsp_touchpad_sens_handle),
                      err, TAG, "Touch sensor enabling failed");
    ...
    for (int i = 0; i < BSP_TOUCHPAD_RETRIES; i++) {
        touch_sensor_trigger_oneshot_scanning(bsp_touchpad_sens_handle, 2000);
    }
    ...
    for (int i = 0; i < BSP_BUTTON_NUM; i++) {
        if (bsp_button_config[i].type == BSP_BUTTON_TYPE_TOUCHPAD) {
            ...
            bsp_touchpad_last_val[chan] = data[0];
        }
    }

    /* Enable and start continuous scanning */
    ESP_GOTO_ON_ERROR(touch_sensor_start_continuous_scanning(bsp_touchpad_sens_handle),
                      err, TAG, "Touch sensor starting failed");
...
err:
    bsp_touchpad_custom_deinit(NULL);
    return ret;
}

static esp_err_t  bsp_touchpad_custom_deinit(button_driver_t *button_driver)
{
    ESP_UNUSED(button_driver);
    if (!bsp_touchpad_sens_initialized) {
        return ESP_OK;
    }
...
static uint8_t bsp_touchpad_custom_get_key_value(button_driver_t *button_driver)
{
    ...
    /* Read filetred value and compare with initial read */
    uint32_t data[TOUCH_SAMPLE_CFG_NUM];
    if (touch_channel_read_data(bsp_touchpad_chan_handles[touchpad_ch], TOUCH_CHAN_DATA_TYPE_SMOOTH, data) != ESP_OK) {
        return 0;
    }
    float ratio = fabs((float)data[0] - bsp_touchpad_last_val[touchpad_ch]) / bsp_touchpad_last_val[touchpad_ch];
    bool pressed = ratio > BSP_TOUCHPAD_THRESH_RATIO;
    if (!pressed) {
        bsp_touchpad_last_val[touchpad_ch] = data[0];
    }
    return pressed ? 1 : 0;
}
...
{
    /* Initialize Touch Sensor buttons, if not */
    BSP_ERROR_CHECK_RETURN_ERR(bsp_touchpad_custom_init());
